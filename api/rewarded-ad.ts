// /api/rewarded-ad.ts — FIX : uuid + points entiers
...
const DEFAULT_CONFIG = {
    points_per_view: 1,      // ✅ ENTIER (int4 ne supporte pas 0.5)
    timer_seconds: 20,
    daily_limit: 15,
    cooldown_seconds: 60,
};
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    // =============================================
    // POST ?action=claim — FIX APPLIQUÉ
    // =============================================
    if (req.method === 'POST' && req.query.action === 'claim') {
        try {
            ...
            // ✅ FIX : Utiliser uuid au lieu de user_id + points entiers
            const pointsReward = Math.floor(session.points || DEFAULT_CONFIG.points_per_view);
            
            // Lire la ligne avec la bonne colonne uuid
            const { data: userData, error: selectError } = await supabase
                .from('user_data')
                .select('points')
                .eq('uuid', user_id)  // ✅ Colonne uuid, pas user_id
                .single();

            console.log('[REWARDED-CLAIM] User lookup:', userData, 'Error:', selectError?.message);
            ...
            if (selectError || !userData) {
                ...
                if (upsertError) {                    console.error('[REWARDED-CLAIM] UPSERT error:', upsertError.message);
                    return res.status(500).json({ error: 'Failed to create user data', details: upsertError.message });
                }
            } else {
                // Mettre à jour uniquement points (int4)
                const newPoints = (userData.points || 0) + pointsReward;
                const { error: updateError } = await supabase
                    .from('user_data')
                    .update({ points: newPoints })
                    .eq('uuid', user_id);  // ✅ Colonne uuid
                
                if (updateError) {
                    console.error('[REWARDED-CLAIM] UPDATE error:', updateError.message);
