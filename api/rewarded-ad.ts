// /api/rewarded-ad.ts — VERSION ULTRA ROBUSTE (sans RPC)
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    // =============================================
    // POST ?action=claim — VERSION ROBUSTE
    // =============================================
    if (req.method === 'POST' && req.query.action === 'claim') {
        try {
            ...
            // ✅ STRATÉGIE ROBUSTE : SELECT → INSERT ou UPDATE
            let currentPoints = 0;
            ...
            try {
                ...
                if (!error && data) {
                    ...
                    console.log('[REWARDED] User exists, current points:', currentPoints);
                } else {
                    console.log('[REWARDED] User not found, will INSERT');
                }
            } catch (e) {
                console.log('[REWARDED] SELECT error:', e);
            }
            ...
            if (userExists) {
                ...
                if (error) {
                    console.error('[REWARDED] UPDATE error:', error.message);
                    // Fallback : essayer avec RPC
                    try {
                        await supabase.rpc('add_user_points', { user_uuid: user_id, points_to_add: pointsReward });
                    } catch (rpcErr) {
                        console.error('[REWARDED] RPC fallback error:', rpcErr);
                ...
                if (error) {
                    console.error('[REWARDED] INSERT error:', error.message);
                    // Fallback : essayer avec RPC
                    try {
                        await supabase.rpc('add_user_points', { user_uuid: user_id, points_to_add: pointsReward });
                    } catch (rpcErr) {
                        console.error('[REWARDED] RPC fallback error:', rpcErr);
                        return res.status(500).json({ error: 'Failed to insert points', details: error.message });
                    }
                }
            }

            // Transaction            await supabase.from('transactions').insert({
                user_id: user_id,
                type: 'earned',
