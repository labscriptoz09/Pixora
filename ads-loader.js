// ads-loader.js v33 — Scripts injectés dans le conteneur pub
(function() {
    'use strict';

    console.log('[ADS] v33 START');
    ...
    function injectHtmlWithScripts(container, html) {
        ...
        // Exécuter les scripts DANS ce conteneur
        var scripts = container.querySelectorAll('script');        scripts.forEach(function(oldScript) {
            var newScript = document.createElement('script');
            for (var i = 0; i < oldScript.attributes.length; i++) {
                newScript.setAttribute(oldScript.attributes[i].name, oldScript.attributes[i].value);
            }
            if (oldScript.innerHTML) newScript.innerHTML = oldScript.innerHTML;
            // Remplacer l'ancien script par le nouveau (dans le même conteneur)
    ...
    async function loadAndInjectAds() {
        ...
        try {
            ...
            if (data2.html && data2.html.trim().length > 0) {
                injectHtmlWithScripts(midSlot.adBox, data2.html);
            } else if (htmlTop) {
                injectHtmlWithScripts(midSlot.adBox, htmlTop);
            } else {
            ...
            if (data3.html && data3.html.trim().length > 0) {
                ...
                injectHtmlWithScripts(btmSlot.adBox, htmlTop);
            } else {
